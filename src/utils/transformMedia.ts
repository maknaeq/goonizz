...
export async function resolveMediaPath(media: Media, query: Record<string, unknown>): Promise<string> {
...
async function resolveImage(mediaPath: string, sourcePath: string, query: Record<string, unknown>): Promise<string> {
    const width = query.width ? Number(query.width) : undefined;
    const height = query.height ? Number(query.height) : undefined;
    const fit = VALID_FITS.includes(query.fit as keyof FitEnum) ? (query.fit as keyof FitEnum) : undefined;
...
async function resolveVideo(media: Media, sourcePath: string, query: Record<string, unknown>): Promise<string> {
...
async function resolveAudio(mediaPath: string, sourcePath: string, query: Record<string, unknown>): Promise<string> {
